...
const OutsideIdentificationForm: FC = (props) => {
    ...
    return (
        <Formik
            initialValues={store.outsideIdentification}
            onSubmit={() => {}}
        >
            {formProps => (
                <div className={'form-section'}>
                    <DropDownContent title={'Идентификаторы во внешних учетных системах'}>
                        <FormArrayField values={formProps.values.outsideIds} name={'outsideIds'} renderChild={
                            (key, index) => (
                                <Row gutter={16}>
                                    <Col span={5}>
                                    ...
                                    <Col span={3}>
                                        <FormField label={'Дата подтверждения'}>
                                            <DatePicker value={moment(formProps.values.outsideIds[index]?.date)} onChange={(_, date) => {
                                                formProps.setFieldValue(`outsideIds[${index}].date`, date)
                                            }}/>
                                        </FormField>
                                    </Col>
                                </Row>
